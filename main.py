...
async def create_processed_agent_data(data: List[ProcessedAgentData]):
    ...
    db.commit()
    db.close()
    return {"message": "Data created successfully and sent to subscribers"}
...
def update_processed_agent_data(processed_agent_data_id: int, data: ProcessedAgentData):
    ...
    db.commit()
    updated_data = db.execute(select(processed_agent_data)
...
def delete_processed_agent_data(processed_agent_data_id: int):
    db = SessionLocal()
    data_to_delete = db.execute(select(processed_agent_data)
                                .where(processed_agent_data.c.id == processed_agent_data_id)).fetchone()
    if data_to_delete is None:
        raise HTTPException(status_code=404, detail="Data not found")
    delete_query = (delete(processed_agent_data)
                    .where(processed_agent_data.c.id == processed_agent_data_id))
